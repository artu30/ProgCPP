...
TList::TList(const TList& list) {
	unsigned int listToCopySize = list.size();

	m_list = new const char*[listToCopySize];

	for (int i = 0; i < listToCopySize; i++) {
		m_list[i] = list.getList()[i];
	}

	m_size = listToCopySize;
	m_needle = 0;
}
...
const char** TList::getList() const{
	return m_list;
}

unsigned int TList::getSize() const {
	return m_size;
}
